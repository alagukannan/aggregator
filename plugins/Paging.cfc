...
Updated for Module settings - Alagukannan Alagappan.
	
To use this plugin you need to create some settings in your coldbox.xml and some
css entries.

COLDBOX SETTINGS/ Module Settings
...
<!------------------------------------------- CONSTRUCTOR ------------------------------------------->	
    ...
    <cffunction name="init" access="public" returntype="Paging" output="false">
		...
		if (len(event.getCurrentModule())){
			  setPagingMaxRows( getModuleSettings(event.getCurrentModule()).settings.PagingMaxRows );
  			  setPagingBandGap( getModuleSettings(event.getCurrentModule()).settings.PagingBandGap );		
		}else{
